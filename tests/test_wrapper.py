...
def test_entry_point_resolves_to_the_real_server():
    ...
    assert callable(avito_mcp_ru.main), "точка входа не вызывается"
    assert avito_mcp_ru.main.__module__.startswith("avito_mcp")
...
def test_versions_agree():
    ...
    assert py == sj["version"] == sj["packages"][0]["version"] == avito_mcp_ru.__version__
...
def test_readme_number_matches_the_catalog_that_ships():
    ...
    assert len(rows) == 64, "каталог изменился, пересобери обвязку генератором"
    assert f"**{len(rows)} " in readme
    assert counts == {"read": 40, "write": 22, "destructive": 2}
...
def test_skill_description_fits_the_spec():
    ...
    text = (ROOT / "skills" / "avito-mcp" / "SKILL.md").read_text(encoding="utf-8")
    head = text.split("---")[1]
